...
class ResultsState extends MusicBeatState
{
    ...
    override public function create()
    {
        ...
        var modOrGame = params.isMod && params.modFolder != null && params.modFolder != "" ? 
                       params.modFolder : "Friday Night Funkin'";
        var now = Date.now();
        var dateStr = LocaleUtils.formatDateTime(now);
        ...
        var playedText = new FlxText(10, 65, infoWidth, 
            Language.getPhrase('results_played_on', 'Played on') + ' $dateStr', 18);
        playedText.setFormat(Paths.font("aller.ttf"), 18, FlxColor.WHITE, "left");
        add(playedText);

        var scoreY = 130;
        var scoreStr = StringTools.lpad("0", "0", 8);
        var scoreLabel = new FlxText(60, scoreY, 400, 
            Language.getPhrase('results_score', 'Score') + ':', 34);
        ...
        flawlesss = new FlxText(leftX, judgY, 340, 
            Language.getPhrase('judgement_flawlesss', 'Flawlesss') + ': 0', 32);
        flawlesss.setFormat(Paths.font("aller.ttf"), 32, 0xFFA17FFF, "left");
        add(flawlesss);

        sicks = new FlxText(rightX, judgY, 340, 
            Language.getPhrase('judgement_sicks', 'Sicks') + ': 0', 32);
        sicks.setFormat(Paths.font("aller.ttf"), 32, 0xFF7FC9FF, "left");
        add(sicks);

        goods = new FlxText(leftX, judgY + judgSpacing, 340, 
            Language.getPhrase('judgement_goods', 'Goods') + ': 0', 32);
        goods.setFormat(Paths.font("aller.ttf"), 32, 0xFF7FFF8E, "left");
        add(goods);

        bads = new FlxText(rightX, judgY + judgSpacing, 340, 
            Language.getPhrase('judgement_bads', 'Bads') + ': 0', 32);
        bads.setFormat(Paths.font("aller.ttf"), 32, 0xFF888888, "left");
        add(bads);

        shits = new FlxText(leftX, judgY + judgSpacing * 2, 340, 
            Language.getPhrase('judgement_shits', 'Shits') + ': 0', 32);
        shits.setFormat(Paths.font("aller.ttf"), 32, 0xFFFF7F7F, "left");
        add(shits);

        misses = new FlxText(rightX, judgY + judgSpacing * 2, 340, 
            Language.getPhrase('judgement_misses', 'Misses') + ': 0', 32);
        misses.setFormat(Paths.font("aller.ttf"), 32, FlxColor.RED, "left");
        add(misses);

        comboText = new FlxText(leftX, judgY + judgSpacing * 3 - 14, 700, 
            Language.getPhrase('judgement_max_combo', 'Highest Combo') + ': 0', 26);
        comboText.setFormat(Paths.font("aller.ttf"), 32, FlxColor.WHITE, "left");
        add(comboText);

        accText = new FlxText(leftX, judgY + judgSpacing * 3 + 20, 700, 
            Language.getPhrase('results_accuracy', 'Accuracy') + ': 0%', 26);
        ...
        if (params.isPractice != null && params.isPractice) {
            var practiceText = new FlxText(0, yBottom, FlxG.width, 
        ...
        var continueText = new FlxText(50, FlxG.height - 75, 0, 
            Language.getPhrase('results_press_enter_mobile', 'Press A\nto Continue'), 26);
        #else
        var continueText = new FlxText(50, FlxG.height - 75, 0, 
    ...
    override public function update(elapsed:Float)
    {
        ...
        if (shouldContinue && allAnimationsComplete())
        {
            #if MODS_ALLOWED
    ...
    function allAnimationsComplete():Bool
    {
        return animatedScore >= params.score &&
               animatedFlawlesss >= params.flawlesss &&
               animatedSicks >= params.sicks &&
               animatedGoods >= params.goods &&
               animatedBads >= params.bads &&
               animatedShits >= params.shits &&
               animatedMisses >= params.misses &&
               animatedCombo >= params.maxCombo &&
